...
public class PropertySetting extends Activity {
		...
		private static final String[] majorItem = { "No", "Baoza","Chioushen","Mentalreset", "護理","照相","運送" };
		private static final String[] majorItem2 = {"No", "包紮","求生","心理輔導", "護理","照相","運送" };
		private static final String[] majorItem3 = { "No","包紮","求生","心理輔導", "護理","照相","運送" };
		private static final String[] career = { "No","Student","Security","Surgeon", "Internist","Fireman","Police","Fire rescue volunteer","Normal volunteer" };
		    	...
		    	b1.setOnClickListener(new Button.OnClickListener()
		        {
					...
					public void onClick(View v) {
							...
							if(!"".equals(editText1.getText().toString().trim())){	//當名字不為空格時
				  				if(!"No".equals(careerSpinner.getSelectedItem().toString().trim())){
				  					if(!"No".equals(majorSpinner1.getSelectedItem().toString().trim()) || 
				  							!"No".equals(majorSpinner2.getSelectedItem().toString().trim()) ||
				  							!"No".equals(majorSpinner3.getSelectedItem().toString().trim())){
				  						flagForCheckDB =true;
				  						showToast("Field blank finish.");
					  				}
				  				}
				  			}
							...
							if("No".equals(careerSpinner.getSelectedItem().toString().trim())){
			  						showToast("Choose occupation");
			  				}
							if("No".equals(majorSpinner1.getSelectedItem().toString().trim()) && 
									"No".equals(majorSpinner2.getSelectedItem().toString().trim()) &&
									"No".equals(majorSpinner3.getSelectedItem().toString().trim())){
		  						showToast("Choose least ability.");
			  				}
							...
							if(flagForCheckDB == true){
								try{
									...
									showToast("Update database sucessful.");
								...
								intent.setClass(PropertySetting.this, MainFunctionActivity.class);
								//intent.setClass(PropertySetting.this, MeteorMapActivity.class);
								startActivity(intent);
								PropertySetting.this.finish();
							}
